...
findInstructors conn = runBeamSqlite conn $ do
  instructors <- runSelectReturningList $ select (all_ (_scheduleInstructor scheduleDb))
  mapM_ (liftIO . putStrLn . show) instructors
...
findTerms conn = runBeamSqlite conn $ do
  ...
  mapM_ (liftIO . putStrLn . show) terms
...
findMarkJones conn = runBeamSqlite conn $ do
  json <- runSelectReturningList $ select $ do
    ...
    return instructors
  mapM_ (liftIO . putStrLn . show) json
...
findTermList  conn = runBeamSqlite conn $ do
  terms <- runSelectReturningList $ select (all_ (_scheduleTerm scheduleDb))
  return terms
